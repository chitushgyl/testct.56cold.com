...
if($pid==0){
    ...
    $parent_id = AdminPermissions::findOne(['id'=>$pid])->parent_id;
}
?>
<div class="layui-card">
    <div class="layui-card-header layuiadmin-card-header-auto">
        <div class="layui-btn-group ">
            <?php if(can('admin.permission.add')){?>
            <a class="layui-btn layui-btn-sm" href="<?php echo route('admin.permission.add','pid='.$pid);?>">添 加</a>
            <?php }?>
    ...
    <div class="layui-card-body">
        ...
        <script type="text/html" id="status">
            <div class="layui-btn-group">
                {{# if(d.status==1){ }}
                    <a class="layui-btn layui-btn-sm" lay-event="">是</a>
                {{# }else{ }}
                    <a class="layui-btn layui-btn-sm layui-btn-disabled" lay-event="">否</a>
                {{# } }}
            </div>
        </script>        
        ...
        <script type="text/html" id="options">
            <div class="layui-btn-group">
                ...
                <?php if(can('admin.permission.edit')){?>
                <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                <?php }?>
                <?php if(can('admin.permission.del')){?>
...
<script>
    ...
    layui.use(['layer','table','form'],function () {
        ...
        var dataTable = table.render({
            elem: '#dataTable'
            ,height: 500
            ,url: "<?php echo route('admin.permission.index')?>"+"pid="+pid //数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
                {field: 'id', title: 'ID',align:'center', width:80}
                ,{field: 'display_name', align:'center',title: '显示名称'}
                ,{field: 'route', align:'center',title: '路由'}
                ,{field: 'icon_id', align:'center',title: '图标', toolbar:'#icon', width:80}
            ...
            if(layEvent === 'del'){
                layer.confirm('确定刪除该权限吗？', function(index){
                    ...
                    $.ajax({
                        ...
                        url:"<?php echo  route('admin.permission.del') ?>",
                        dataType:"JSON",
                        type:"POST",
                        beforeSend:function(){
                ...
                location.href = '/admin/permission/edit?id='+data.id;
            } else if (layEvent === 'children'){
                location.href = '/admin/permission/index?pid='+data.id;
            }
        });
        ...
        $("#returnParent").click(function () {
            ...
            location.href = '/admin/permission/index?pid='+ pid;
        })
    })
</script>
